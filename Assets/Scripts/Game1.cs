...
public class Game1 : MonoBehaviour
{
    ...
    public int allBonusPS = 0;
    public int allBonPS = 0;
    ...
    private void Awake()
    {
        /*if (PlayerPrefs.HasKey("SV"))
        {
            ...
            print("Ваши рабочие заработали: " + offlineBonus + " ϟ");
        }*/
    }
    ...
    private void Update()
    {
        scoreText.text = score + " ϟ"; //Отображаем деньги
        allBonusPS = shopItems[8].bonusPerSec * shopItems[8].bonusCounter
            + shopItems[9].bonusPerSec * shopItems[9].bonusCounter
            + shopItems[10].bonusPerSec * shopItems[10].bonusCounter
            + shopItems[11].bonusPerSec * shopItems[11].bonusCounter
            + shopItems[12].bonusPerSec * shopItems[12].bonusCounter
            + shopItems[13].bonusPerSec * shopItems[13].bonusCounter
            + shopItems[14].bonusPerSec * shopItems[14].bonusCounter
            + shopItems[15].bonusPerSec * shopItems[15].bonusCounter;
    ...
    private void updateCosts() // Метод для обновления текста с ценами
    {
        for (int i = 0; i < shopItems.Count; i++) // Цикл выполняется, пока переменная i < кол-ва товаров
        {
            if (shopItems[i].itsBonus) // Если товар является бонусом, то:
            {
                ...
                shopItemsText[i].text = shopItems[i].name + "\n" + cost + " ϟ"; // Обновляем текст кнопки с рассчитанной ценой
            }
            else shopItemsText[i].text = shopItems[i].name + "\n" + shopItems[i].cost + " ϟ"; // Иначе если товар не является бонусом, то обновляем текст кнопки с обычной ценой
        }
    }   
    ...
    IEnumerator BonusTimer(float time, int index) // Бонусный таймер (длительность бонуса (в сек.),индекс товара)
    {
         ...
         shopItems[shopItems[index].itemIndex].bonusPerSec += allBonPS; // Удваиваем бонус рабочих в секунду (к примеру)
         yield return new WaitForSeconds(time); // Делаем задержку на столько секунд, сколько указали в параметре
         shopItems[shopItems[index].itemIndex].bonusPerSec -= allBonPS; // Возвращаем бонус в нормальное состояние
